...
Robustness fixes vs the first cut:
  * Fetching is button-driven and persisted in session_state (no brittle
    double-caching). Scoring is recomputed live from the cached feature table,
    so the weight sliders re-rank instantly without re-downloading.
  * The table's color gradient needs matplotlib; if it's missing we fall back
    to a plain table instead of erroring.
  * The Styler format map is filtered to columns that actually exist.
"""
...
# ---------------------------------------------------------------------------
# Fetch (button-driven; persisted in session_state)
# ---------------------------------------------------------------------------
...
sig = (universe_kind, top_n, with_news)

# ---------------------------------------------------------------------------
# Sidebar — live scoring knobs
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Header + fetch trigger
# ---------------------------------------------------------------------------
st.title("🇮🇳 NSE NIFTY Buy-Zone / Upside Screener")
st.caption("Ranks NIFTY large caps on analyst upside, buy-zone technicals, "
           "sector-relative valuation & quality, and momentum. Prices in ₹. "
           "Not financial advice.")

if run or force or "feats" not in st.session_state or st.session_state.get("sig") != sig:
    with st.spinner("Resolving universe & downloading data (first run is slow)..."):
        try:
            prices, funds, feats = fetch(universe_kind, top_n, with_news, workers, force)
            st.session_state.update(prices=prices, funds=funds, feats=feats, sig=sig)
...
if feats is None or feats.empty:
    st.warning("No data yet. Click **Fetch data** in the sidebar. "
               "(NSE fundamentals/analyst coverage via Yahoo is thinner than the "
               "US — some cells may be blank.)")
    st.stop()

# ---------------------------------------------------------------------------
# Score (cheap, reactive to sliders)
# ---------------------------------------------------------------------------
...
with tab_rank:
    ...
    try:  # gradient needs matplotlib; degrade gracefully if absent
    ...
    dl1.download_button("⬇️ CSV", ranked.to_csv().encode(),
                        "nse_screen.csv", "text/csv", use_container_width=True)
    try:
        dl2.download_button(
...
with tab_detail:
    ...
    pick = st.selectbox("Ticker", ranked.index.tolist(),
    ...
    with dr:
        ...
        st.write({
            ...
            "Rev growth": f"{row['rev_growth']:.1%}" if pd.notna(row["rev_growth"]) else None,
